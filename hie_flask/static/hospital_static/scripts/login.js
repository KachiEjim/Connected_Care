document.addEventListener('DOMContentLoaded', function() {
    ...
    const baseURL = 'http://192.168.32.212';
        ...
        if (isValid) {
            ...
            validateUser(email.value, password.value).then(response => {
                if (!response.email) {
                    ...
                    showError(email, `Hospital not found`);
                    errorMessage.innerHTML = `It looks like you are not registered yet, no worries <a href="${baseURL}:5000/patient/signup">Sign up here!</a>`;
